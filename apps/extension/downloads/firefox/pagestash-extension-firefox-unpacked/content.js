...
const UserSchema = objectType({
    ...
    subscription_tier: enumType(['free', 'pro']).default('free'),
...
const ClipSchema = objectType({
    ...
    url: stringType().url(),
    title: stringType(),
    screenshot_url: stringType().url().optional(),
    html_content: stringType().optional(),
    text_content: stringType().optional(),
    favicon_url: stringType().url().optional(),
    created_at: stringType().datetime(),
    updated_at: stringType().datetime(),
    folder_id: stringType().uuid().optional(),
...
const FolderSchema = objectType({
    ...
    parent_id: stringType().uuid().optional(),
    created_at: stringType().datetime(),
    updated_at: stringType().datetime(),
    color: stringType().optional(),
});
// Tag types
const TagSchema = objectType({
    id: stringType().uuid(),
    user_id: stringType().uuid(),
    name: stringType().min(1).max(50),
    color: stringType().optional(),
    created_at: stringType().datetime(),
});
// Clip-Tag relationship
const ClipTagSchema = objectType({
    clip_id: stringType().uuid(),
    tag_id: stringType().uuid(),
    created_at: stringType().datetime(),
});
// API request/response types
const CreateClipRequestSchema = objectType({
    url: stringType().url(),
    title: stringType(),
    screenshot_data: stringType().optional(), // base64 encoded image
    html_content: stringType().optional(),
    text_content: stringType().optional(),
    favicon_url: stringType().url().optional(),
    folder_id: stringType().uuid().optional(),
    tag_ids: arrayType(stringType().uuid()).optional(),
    notes: stringType().optional(),
});
const UpdateClipRequestSchema = objectType({
...
// Search types
const SearchRequestSchema = objectType({
    query: stringType().optional(),
    folder_id: stringType().uuid().optional(),
    tag_ids: arrayType(stringType().uuid()).optional(),
    limit: numberType().min(1).max(100).default(20),
    offset: numberType().min(0).default(0),
    sort_by: enumType(['created_at', 'updated_at', 'title', 'relevance']).default('created_at'),
    sort_order: enumType(['asc', 'desc']).default('desc'),
});
const SearchResponseSchema = objectType({
    clips: arrayType(ClipSchema),
    total_count: numberType(),
    has_more: booleanType(),
});
// Extension message types
const ExtensionMessageSchema = discriminatedUnionType('type', [
    objectType({
        type: literalType('CAPTURE_PAGE'),
        payload: objectType({
            url: stringType().url(),
            title: stringType(),
            captureType: enumType(['visible', 'fullPage']).optional(),
            html: stringType().optional(),
            text: stringType().optional(),
            favicon: stringType().url().optional(),
        }),
    }),
    objectType({
        type: literalType('CAPTURE_PROGRESS'),
        payload: objectType({
            status: stringType(),
            message: stringType(),
        }),
    ...
    objectType({
        type: literalType('CAPTURE_SUCCESS'),
        payload: objectType({
            synced: booleanType(),
    ...
    objectType({
        type: literalType('CAPTURE_ERROR'),
        payload: objectType({
            error: stringType(),
        }),
    }),
    objectType({
        type: literalType('GET_AUTH_TOKEN'),
        payload: objectType({}),
    }),
    objectType({
        type: literalType('AUTH_TOKEN_RESPONSE'),
        payload: objectType({
            token: stringType().optional(),
        }),
    }),
    objectType({
        type: literalType('CANCEL_CAPTURE'),
        payload: objectType({}),
    }),
    objectType({
        type: literalType('EXTRACT_PAGE_DATA'),
        payload: objectType({}),
    }),
]);
// Error types
const ApiErrorSchema = objectType({
    error: stringType(),
    message: stringType(),
...
function cleanHtmlContent(html) {
    // This is a basic implementation - in production, we'd use a proper HTML sanitizer
    return html
