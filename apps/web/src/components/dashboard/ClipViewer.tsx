...
export function ClipViewer({ 
  ...
  const formatNotesDisplay = (notes: string) => {
    ...
    result = result.replace(/📍 SCREENSHOT \[x:(\d+),y:(\d+),w:(\d+),h:(\d+)(?:,thumb:([^\]]*))?\]\n([^\n📍]+(?:\n(?!📍)[^\n]+)*)/g, (match, x, y, w, h, thumb, noteText) => {
      ...
      // Clean, minimal HTML with proper display block
      if (thumb && thumb.trim()) {
        // With thumbnail - no border on image to prevent visual artifacts
        const badge = `<div class="mb-2 bg-purple-50 border border-purple-200 rounded-md overflow-hidden block"><div class="relative group"><div class="flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-purple-100 transition-colors" onclick="window.toggleAnnotation?.(${index}, event)"><div class="relative flex-shrink-0"><img src="${thumb}" alt="" class="w-8 h-8 rounded object-cover block" style="display:block;" /><div class="absolute left-full ml-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-50" style="white-space:nowrap;"><div class="bg-white border-2 border-purple-300 rounded-lg shadow-xl p-1"><img src="${thumb}" alt="" class="w-32 h-32 rounded object-cover block" style="display:block;" /></div></div></div><span class="text-xs font-medium text-purple-700 flex-1">Screenshot Annotation</span><svg class="w-4 h-4 text-purple-600 transition-transform flex-shrink-0${isExpanded ? ' rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div><button onclick="window.navigateToAnnotation?.(${index}, ${x}, ${y}, event)" class="absolute right-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 bg-purple-600 hover:bg-purple-700 text-white text-xs rounded" title="Jump"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></button></div>${isExpanded ? `<div class="px-3 py-2 text-sm text-gray-700 border-t border-purple-200 bg-white" style="display:block;">${noteText.trim().replace(/\n/g, '<br>')}</div>` : ''}</div>`
        return badge
      } else {
        // Without thumbnail - with icon
        const badge = `<div class="mb-2 bg-purple-50 border border-purple-200 rounded-md overflow-hidden block"><div class="relative group"><div class="flex items-center gap-1.5 px-2 py-1.5 cursor-pointer hover:bg-purple-100 transition-colors" onclick="window.toggleAnnotation?.(${index}, event)"><svg class="w-3.5 h-3.5 text-purple-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg><span class="text-xs font-medium text-purple-700 flex-1">Screenshot Annotation</span><svg class="w-4 h-4 text-purple-600 transition-transform flex-shrink-0${isExpanded ? ' rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div><button onclick="window.navigateToAnnotation?.(${index}, ${x}, ${y}, event)" class="absolute right-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 bg-purple-600 hover:bg-purple-700 text-white text-xs rounded" title="Jump"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></button></div>${isExpanded ? `<div class="px-3 py-2 text-sm text-gray-700 border-t border-purple-200 bg-white" style="display:block;">${noteText.trim().replace(/\n/g, '<br>')}</div>` : ''}</div>`
        return badge
      }
    })
