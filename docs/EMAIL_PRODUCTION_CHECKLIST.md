...
**Why This Matters:**
- After email confirmation, users are sent to this URL
- The callback route exchanges the auth code for a session
- Then redirects to `/dashboard` where users can start using the app
...
**Redirect URL field:** (This is DIFFERENT from the URL Configuration page)
```
{{ .SiteURL }}/auth/callback?next=/auth/reset-password
```

**Why This Matters:**
- This tells Supabase WHERE to send users when they click the email link
- The `{{ .SiteURL }}` will be replaced with your configured Site URL (`https://pagestash.app`)
- The callback route exchanges the auth code for a session
- Then redirects to `/auth/reset-password` where users enter their new password

**Common Mistake:** If you leave this blank or wrong, users get redirected to the homepage instead of the reset password page.
...
**Why This Matters:**
- After clicking the magic link, users are sent to this URL
- The callback route exchanges the auth code for a session
- Then redirects to `/dashboard` where users are logged in
...
| **Password reset → homepage** | **Missing Redirect URL in email template** | **See critical fix below** |

**🚨 CRITICAL FIX: Password Reset Redirects to Homepage Instead of Reset Page**

**Problem:**
- User clicks "Reset Password" link in email
- Gets redirected to homepage instead of `/auth/reset-password`
- Can't enter new password

**Root Cause:**
The "Redirect URL" field in the Supabase email template is blank or misconfigured. This is DIFFERENT from the "Redirect URLs" whitelist on the URL Configuration page.

**Solution:**
1. Go to: **Supabase Dashboard → Authentication → Email Templates → Reset Password**
2. Scroll to the **"Redirect URL"** field (below the HTML template editor)
3. Enter exactly:
   ```
   {{ .SiteURL }}/auth/callback?next=/auth/reset-password
   ```
...
5. Test the flow:
   ...
   - Request reset
   - Click link in email
   - Should redirect to `/auth/reset-password` (not homepage)

**Why This Works:**
- `{{ .SiteURL }}` = Your configured Site URL (`https://pagestash.app`)
- `/auth/callback` = Exchange auth code for session
- `?next=/auth/reset-password` = Then redirect to reset password page

**Same Fix Applies To:**
- **Confirm Signup**: Use `{{ .SiteURL }}/auth/callback?next=/dashboard`
- **Magic Link**: Use `{{ .SiteURL }}/auth/callback?next=/dashboard`
...
- [ ] 3. **Configure Redirect URLs in EACH email template (CRITICAL):**
  - [ ] 3a. Confirm Signup: `{{ .SiteURL }}/auth/callback?next=/dashboard`
  - [ ] 3b. Reset Password: `{{ .SiteURL }}/auth/callback?next=/auth/reset-password`
  - [ ] 3c. Magic Link (if used): `{{ .SiteURL }}/auth/callback?next=/dashboard`
- [ ] 4. Configure custom SMTP (SendGrid/AWS SES/Resend)
- [ ] 5. Copy custom HTML templates to Supabase Email Templates
- [ ] 6. Update support email to `support@pagestash.app` in all templates
- [ ] 7. Test signup email end-to-end
- [ ] 8. Test password reset email end-to-end
- [ ] 9. Test emails in Gmail, Outlook, Apple Mail
- [ ] 10. Test on mobile devices
- [ ] 11. Configure SPF/DKIM records (if using custom domain)
- [ ] 12. Set up email monitoring/alerts
- [ ] 13. Document support process for "email not received" issues
